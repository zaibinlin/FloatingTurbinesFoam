...
bug=100   #prevent different columns in different files when breakdown
print('Each time instant has {} entry (entries).'.format(nEntry))
if nEntry > 1:
  reduceRow = [0, *range(nEntry, len(data[:,0])-bug, nEntry)]  #len(data[:,0]
  data = data[reduceRow,:]
  print('Data array rows reduced.')
  time = data[:,0]

print("Time range in output file: ", time[0], " - ", time[-1])
npoints = int(len(data[0,1:])/4+1)
...
for i, ti in enumerate(time):
  if abs(tselect-ti).min() < 1e-5:
    ...
    points = np.reshape(data[i,1:npoints*3+1], (-1,3))
    for p in points:
    ...
    if nlines>1:
      for data1 in otherData:
        points1 = np.reshape(data1[i,1:npoints*3+1], (-1,3))
        for p in points1:
          file1.write(' '.join('{:.3f}'.format(px) for px in p))
          file1.write('\n')
    ...
    #Tension
    file1.write("\nPOINT_DATA {}".format(nlines * npoints))
    file1.write("\nSCALARS Tension(N) float")
    file1.write("\nLOOKUP_TABLE default\n")
    tension = np.reshape(data[i, npoints * 3 + 1:], (-1, 1))
    for p in tension:
      file1.write(' '.join('{:.3f}'.format(px) for px in p))
      file1.write('\n')
    if nlines>1:
      for data1 in otherData:
        tension1 = np.reshape(data1[i,npoints * 3 + 1:], (-1,1))
        for p in tension1:
          file1.write(' '.join('{:.3f}'.format(px) for px in p))
          file1.write('\n')
    file1.close()
